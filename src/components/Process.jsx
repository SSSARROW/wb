...
const OurProcess = () => {
  ...
  <section className="text-white relative py-20" id="process">
        ...
        <div className="w-full flex flex-col md:flex-row items-center justify-center mb-8 pt-8">
          <div className="relative w-full max-w-xl h-4 flex items-center">
            ...
            <motion.div
              ...
              style={{ width: `${(currentStep / (processSteps.length - 1)) * 100}%` }}
              transition={{ type: "spring", stiffness: 120, damping: 25, duration: 1.2 }}
            ></motion.div>
            {processSteps.map((step, idx) => (
              <motion.div
                key={idx}
                className={`absolute top-1/2 -translate-y-1/2 rounded-full border-2 ${
                  idx <= currentStep ? "border-green-500 bg-green-500" : "border-zinc-400 bg-zinc-800"
                }`}
                style={{ left: `calc(${(idx / (processSteps.length - 1)) * 100}% - 10px)`, width: "20px", height: "20px", zIndex: 2 }}
                transition={{ type: "spring", stiffness: 120, damping: 25, duration: 1.2 }}
              >
                ...
                <AnimatePresence mode="wait">
                  {isActive && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -30 }}
                      transition={{ type: "spring", stiffness: 80, damping: 18, duration: 0.8, ease: "easeInOut" }}
                      className="mt-6 px-4"
                    >
                      <h3 className="text-2xl md:text-3xl font-bold text-white mb-4">{step.title}</h3>
                      <p className="text-base text-zinc-400 max-w-md mx-auto leading-relaxed">{step.description}</p>
                    </motion.div>
                  )}
                </AnimatePresence>
              </motion.div>
      ...
      <div className="mt-16 text-center">
        <p className="mb-6 font-semibold text-green-400">Swipe or use your mouse wheel to explore.</p>
